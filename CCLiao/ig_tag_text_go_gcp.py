...
def get_texts(html):
    ...
    while flag and len(tag_post_data) < 2000:
        ...
        print(cursor, flag)
        time.sleep(5 + float(random.randint(1, 800))/200)    # if count > 2000, turn on
    return tag_post_data
...
def main(tag):
    ...
    if len(tag_texts):
        ...
        dirpath = './ig_text_download/'
        if not os.path.exists(dirpath):
            os.makedirs(dirpath)

        tag_texts.to_csv('{}/{}.csv'.format(dirpath, tag), encoding='utf-8')

if __name__ == '__main__':
    # tag_name = '天主教臺灣總修院'
    start = time.time()
    # main(tag_name)
    df = pd.read_csv('./bigdict.csv', encoding='utf-8')
    tag_text_list = df['value'].tolist()[500:1000]

    for tag_name in tag_text_list:
        main(tag_name.strip())
        time.sleep(60 + float(random.randint(1, 800)) / 200)
    print('Complete!!!!!!!!!!')
    end = time.time()
    spend = end - start
