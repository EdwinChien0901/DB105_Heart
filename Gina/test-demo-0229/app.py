...
# 引用套件
from linebot.models import (
    MessageEvent, TextMessage, TextSendMessage,  PostbackEvent, ImageMessage, ImagemapSendMessage
)
...
#         with open("./images/{0}.jpg".format(event.reply_token), 'wb') as fd:
#             for chunk in message_content.iter_content():
#                 fd.write(chunk)

#         img1 = cv2.imread("./images/{0}.jpg".format(event.reply_token), 1)
...
#         file = util.getRedisImg(event.reply_token)
        ...
        line_bot_api.reply_message(
            event.reply_token,
            TextSendMessage(text="好棒！相似度很高！恭喜您，已解鎖您的旅遊成就！")
        )
        ...
        elif event.message.text=="淡水老街":  #觸發 
...
# # 引用套件
# from linebot.models import (
#     MessageEvent, TextMessage, TextSendMessage,  PostbackEvent, ImageMessage
# )
